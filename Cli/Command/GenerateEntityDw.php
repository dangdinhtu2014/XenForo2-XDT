...
class GenerateEntityDw extends Command
{
	...
	{
		$this
			->setName('xdt:generate-entity-dw')
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		foreach ($fields AS $columnName => $field)
		{
			...
			if (!empty($field['allowedValues']))
			{
				...
				{
					$allowed[] = floatval($field['default']);
				}
				else
				{
					$allowed[] = "'" . addslashes($field['default']) . "'";
				}

				$record[] = "\n\t\t\t\t'allowedValues' => array(" . implode(', ', $allowed) . ")";
			}
			$code = implode(', ', $record);
			$trailingBreak = (strpos($code, "\n") ? "\n\t\t\t" : '');
			$columns[] = "'$columnName' => array($code$trailingBreak)";
		}
...
ENTITYOUT;
		...
		{
			$fileName = \XF::getAddOnDirectory()
				. DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, $namespace)
